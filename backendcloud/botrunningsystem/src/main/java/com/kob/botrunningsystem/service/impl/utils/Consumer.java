...
public class Consumer extends Thread {
    ...
    private String addUid(String code, String uid) {//在code中的Bot类名后加uid
        int k = code.indexOf(" implements java.util.function.Supplier<Integer>");//在这个字符串前面加
    ...
    public void run() {
        ...
        //在这里编译执行代码
        Supplier<Integer> botInterface = Reflect.compile(//joor里动态编译一段代码
                "com.kob.botrunningsystem.utils.Bot" + uid,
                addUid(bot.getBotCode(), uid)
        ).create().get();//编译完后创建一个类再获取一下

        File file = new File("input.txt");
        try (PrintWriter fout = new PrintWriter(file)) {//把bot.getInput()写进file里（一行字符串）
        ...
        Integer direction = botInterface.get();//调用动态编译后代码里的方法
